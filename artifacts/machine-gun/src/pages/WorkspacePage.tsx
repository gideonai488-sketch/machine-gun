...
import { Eye, Code2, Share2, Rocket, Loader2, MessageSquare } from 'lucide-react'
...
function WorkspaceContent() {
  ...
  useEffect(() => {
    function handleProjectUpdated(data: any) {
      if (data.previewUrl) setPreviewUrl(data.previewUrl)
      if (data.status) setBuildStatus(data.status)
    }
    function handleFilesUpdated(data: any) { setFiles(data.files) }
    function handleFileChanged(data: any) { setFileContent(data.path, data.content) }

    socket.on('project:updated', handleProjectUpdated)
    socket.on('files:updated', handleFilesUpdated)
    socket.on('file:changed', handleFileChanged)
    socket.on('build:status', (data: any) => setBuildStatus(data.status))
    return () => {
    ...
    <div className="h-screen h-screen-safe flex flex-col overflow-hidden bg-white">
      <header className="h-11 sm:h-12 border-b border-slate-200 bg-white flex items-center justify-between px-3 sm:px-4 shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
          ...
          <Button size="sm" className="h-7 text-[11px] sm:text-xs gap-1 px-2.5 sm:px-3 bg-red-500 hover:bg-red-600 text-white border-0" onClick={() => setDeployOpen(true)}>
