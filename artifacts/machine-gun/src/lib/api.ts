...
export const api = {
  ...
  sendMessage(projectId: string, message: string) {
    return request(`/api/projects/${projectId}/chat`, { method: 'POST', body: JSON.stringify({ message }) })
  },
  ...
  saveFile(projectId: string, path: string, content: string) {
    return request(`/api/projects/${projectId}/files/content`, { method: 'PUT', body: JSON.stringify({ path, content }) })
  },
  deployWeb(projectId: string) {
    return request(`/api/projects/${projectId}/deploy/web`, { method: 'POST' })
  },
  deployMobile(projectId: string, platform: string, track: string) {
    return request(`/api/projects/${projectId}/deploy/mobile`, { method: 'POST', body: JSON.stringify({ platform, track }) })
  },
