...
import { X, FileCode, ChevronRight, ChevronDown, File, Folder, FolderOpen, PanelLeft } from 'lucide-react'
...
function FileNode({ node, depth = 0, onSelect, activeFile }: { node: any; depth?: number; onSelect: (p: string) => void; activeFile: string | null }) {
  ...
  return (
    <div>
      ...
      >
        {isDir ? (
          <>
            ...
            {expanded ? <FolderOpen className="w-3.5 h-3.5 text-red-400 shrink-0" /> : <Folder className="w-3.5 h-3.5 text-slate-400 shrink-0" />}
          </>
        ) : (
          <>
      ...
      {isDir && expanded && node.children?.map((child: any) => (
    ...
    <div className="h-full flex relative bg-white">
      <div className={cn(
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200">
          ...
          <button onClick={() => setShowTree(false)} className="p-1 rounded hover:bg-slate-100 sm:hidden cursor-pointer">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex items-center border-b border-slate-200 bg-slate-50/50 overflow-x-auto shrink-0 no-scrollbar">
          ...
          {openFiles.map((filePath: string) => (
            <div
              key={filePath}
              className={cn(
                'flex items-center gap-1.5 px-3 py-2 text-[11px] border-r border-slate-200 cursor-pointer transition-colors shrink-0 group',
                activeFile === filePath ? 'bg-white text-slate-900' : 'text-slate-400 hover:text-slate-600 hover:bg-white'
              )}
