...
import { Globe, Smartphone, Apple, Rocket, Loader2, CheckCircle2, XCircle, ExternalLink, X } from 'lucide-react'
...
const DEPLOY_OPTIONS = [
  { id: 'web', label: 'Publish Website', description: 'Build and deploy to a live URL', icon: Globe, color: 'text-blue-400', bg: 'bg-blue-500/10' },
  { id: 'android', label: 'Google Play', description: 'Build AAB and publish to Play Store', icon: Smartphone, color: 'text-green-400', bg: 'bg-green-500/10' },
  { id: 'ios', label: 'App Store', description: 'Build IPA and publish via TestFlight', icon: Apple, color: 'text-slate-600', bg: 'bg-slate-100' },
]
...
export default function DeployPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    function handleDone(data: any) {
      setDeploying(null); setStatus('success')
      if (data.url) setLiveUrl(data.url)
    }
    function handleError(data: any) {
      setDeploying(null); setStatus('error'); setError(data.message)
    }
    function handleDeployStatus(data: any) {
  ...
  async function handleDeploy(type: string) {
    if (!project?.id || deploying) return
    setDeploying(type); setStatus(null); setLiveUrl(null); setError(null)
    try {
      if (type === 'web') {
      ...
      setDeploying(null); setStatus('error'); setError(err.message)
    }
  }

  function reset() {
    setDeploying(null); setStatus(null); setLiveUrl(null); setError(null)
  }
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Publish your app to the world</p>
            </div>
          </div>
        ...
        {status === 'success' && liveUrl ? (
          <div className="px-5 pb-5">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
              ...
              <a href={liveUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-red-500 hover:underline flex items-center justify-center gap-1">
                {liveUrl}<ExternalLink className="w-3 h-3" />
              </a>
            </div>
            <Button onClick={reset} variant="outline" className="w-full mt-3 h-9 text-xs">Deploy Again</Button>
          </div>
        ) : status === 'success' ? (
          <div className="px-5 pb-5">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
            ...
            <Button onClick={reset} variant="outline" className="w-full mt-3 h-9 text-xs">Done</Button>
          </div>
        ) : status === 'error' ? (
          <div className="px-5 pb-5">
            ...
            <Button onClick={reset} variant="outline" className="w-full mt-3 h-9 text-xs">Try Again</Button>
          </div>
        ) : (
                ...
                <div key={opt.id}>
                  <button
                    ...
                    className={cn(
                      ...
                      isDeploying ? 'bg-red-50/50 border-red-200' : 'hover:bg-slate-50 hover:border-slate-300 hover:shadow-sm',
                      isDisabled && 'opacity-40 pointer-events-none'
                    )}
                  >
                    <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shrink-0', opt.bg)}>
                      {isDeploying ? <Loader2 className="w-5 h-5 text-red-500 animate-spin" /> : <Icon className={cn('w-5 h-5', opt.color)} />}
                    </div>
                    <div className="text-left flex-1 min-w-0">
                      <p className="font-medium text-sm">{opt.label}</p>
                      <p className="text-[11px] text-slate-400">{opt.description}</p>
                    </div>
                    {isDeploying && <span className="text-[10px] text-red-500 font-medium shrink-0">Deploying...</span>}
                  </button>
                      ...
                      {ANDROID_TRACKS.map((t) => (
                        <button
                          ...
                          className={cn(
                            ...
                            androidTrack === t.id ? 'bg-red-50 text-red-600 font-medium' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'
                          )}
                        >
                          {t.label}
                        </button>
