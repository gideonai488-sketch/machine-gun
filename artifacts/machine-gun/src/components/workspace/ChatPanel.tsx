...
import {
  Send, User, Loader2, FileCode, Package, CheckCircle2,
  XCircle, Wrench, Eye, Search, Rocket,
} from 'lucide-react'
...
function ActivityBubble({ message }: { message: any }) {
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-red-500/20 focus-within:border-red-300 transition-all shadow-sm">
            ...
            <div className="flex items-center justify-end px-2 pb-1.5 sm:pb-2">
              <Button
                ...
                className="h-8 w-8 sm:h-7 sm:w-7 rounded-lg bg-red-500 hover:bg-red-600 text-white border-0"
              >
                {isStreaming ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                ) : (
